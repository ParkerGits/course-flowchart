import { DEFAULT_MAX_CREDITS } from './../pages/index'
...
export class Graph {
	...
	constrainedOrdering(
		...
		if (
			this.constrainedFrames[startTerm] &&
			this.courseSchedule[startTerm] &&
			maxCredits == 15
		) {
			return {
				courseSchedule: this.courseSchedule[startTerm]!,
				frames: this.constrainedFrames[startTerm]!,
			}
		}

		// Get data for each course in topological ordering.
		if (!this.courseOrder) {
			this.courseOrder = this.topOrder.map(
		...
		const coursesByTerm: { [key in Term]: JSONCourse[] } = {
			fall: [],
			winter: [],
			spring: [],
		}
		...
		// cache results for the DEFAULT_MAX_CREDITS value
		// storing results for all values of maxCredits uses lots of memory
		// speed/memory tradeoff
		if (maxCredits === DEFAULT_MAX_CREDITS) {
	...
	// Generates links for all nodes in the graph
	links(): Link[] {
	...
	// Generates links for nodes that are part of the current frame
	...
	private topologicalOrdering(): string[] {
		...
		this.edges.forEach((neighbors) => {
			neighbors.forEach((neighbor) => {
				if (!incomingActiveEdges.has(neighbor))
					incomingActiveEdges.set(neighbor, 1)
				else
					incomingActiveEdges.set(
						neighbor,
						incomingActiveEdges.get(neighbor)! + 1,
					)
			})
		})
