...
export default function Home({ majorCourses }: { majorCourses: MajorCourses }) {
  ...
  const [selectedMajor, setSelectedMajor] = useState<keyof MajorCourses>(majors[1]);
  ...
  const graphs: MajorGraphs = useMemo(() => constructMajorGraphs(majorCourses), [majorCourses]);
  ...
  return (
    <div className="w-screen min-w-fit">
      ...
      <TextOutput graph={graph} isConstrained={isConstrained} startTerm={startTerm} maxCredits={maxCredits} />
    </div>
  );
}
